...
auto CreateRoundedButtonOption() -> ftxui::ButtonOption {
  ...
  option.transform = [](const ftxui::EntryState &s) {
    ...
    return ftxui::borderRounded(
        ftxui::hbox({ftxui::filler(), text_element, ftxui::filler()}));
  };
  return option;
}
...
void DataBaseExplorer<Connector>::Ininitalize() {
  req_input = ftxui::Input(&req_text, "Enter SQL request") |
      ...
      [this] {
        ...
        auto process_res = [this](std::expected<Table, DbError> res) {
          if (res.has_value()) {
            ...
            max_page =
                db_result.empty() ? 0 : (db_result.size() - 1) / rows_per_page;
          } else {
            db_result.clear();
        ...
        if (copy.find("SELECT") != std::string::npos) {
          ...
          if (!clean_req.empty() && clean_req.back() == ';')
            clean_req.pop_back();
  ...
  table_component = ftxui::Renderer([this] {
    ...
    return rendered_table_cache |
           ftxui::focusPositionRelative(scroll_x, scroll_y) | ftxui::frame |
...
template <DatabaseConnection Connector>
std::vector<ftxui::Elements> DataBaseExplorer<Connector>::FormatTable(
    const Table &table, size_t current_page, const size_t rows_per_page) {
  ...
  for (size_t i = (current_page == 0 ? 0 : start_index + 1); i <= end_index;
       i++) {
