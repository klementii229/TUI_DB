...
SQLiteConnector::Connect(const std::string &connectionString) {
  if (sqlite3_open(connectionString.c_str(), &db) == SQLITE_OK) {
    return {};
  }
  std::string err_details =
      db ? sqlite3_errmsg(db) : "Failed to allocate memory for SQLite";
  if (db) {
    sqlite3_close(db);
    db = nullptr;
  }
...
std::expected<Table, DbError>
SQLiteConnector::FetchAll(const std::string &query) {
  Table result;
